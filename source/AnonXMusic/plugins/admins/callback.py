...
@app.on_callback_query(filters.regex("developers") & ~BANNED_USERS)
async def devs_back(c, query):
    if query.message.chat.type != ChatType.CHANNEL:
        print(str(query.from_user.id), (query.data.strip()).split()[1])
        if str(query.from_user.id) != str((query.data.strip()).split()[1]):
            if query.from_user.id not in SUDOERS:
                return await query.answer("الامر ليس لك")
    else:
        admins = adminlist.get(query.message.chat.id)
        if not admins:
            return await query.answer("لازم تكون ادمن ,لو انت ادمن جرب تعمل ريلود", show_alert=True)
        else:
            if query.from_user.id not in admins:
                return await query.answer(
                    "لازم تكون ادمن ,لو انت ادمن جرب تعمل ريلود", show_alert=True
                )
    kb = InlineKeyboardMarkup([[InlineKeyboardButton(text=". Dev¹ .",url="https://t.me/x_hseein"),InlineKeyboardButton(text=". Dev² .",url="https://t.me/X_zazthon")],[InlineKeyboardButton(text=". Back .", callback_data=f"return-developer {query.from_user.id}")],])
    return await query.edit_message_text("‹ Welcome To Zazthon Source ›\n\n - Dev ↝ @x_hseein\n - Dev ↝ @X_zazthon", reply_markup=kb)

@app.on_callback_query(filters.regex("return-developer") & ~BANNED_USERS)
async def devs_retrun_back(c, query):
    if query.message.chat.type != ChatType.CHANNEL:
        if str(query.from_user.id) != str((query.data.strip()).split()[1]):
            if query.from_user.id not in SUDOERS:
                return await query.answer("الامر ليس لك")
    ...
    return await query.edit_message_text(f"• 𝗧𝗵𝗲 𝗕𝗲𝘀𝘁 𝗦𝗼𝘂𝗿𝗰𝗲 𝗢𝗻 𝗧𝗲𝗹𝗲𝗴𝗮𝗺 🎸 .", reply_markup=
           InlineKeyboardMarkup([[
               InlineKeyboardButton("- 𝖲𝗈𝖴𝖱𝖢𝖾 .", url=SUPPORT_CHANNEL),
               InlineKeyboardButton("- Help .", url=SUPPORT_CHAT),
               ],[
               InlineKeyboardButton("- DeVeLoPeRS .", callback_data=f"developers {query.from_user.id}"),
               ],]),)
