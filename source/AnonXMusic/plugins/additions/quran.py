...
quran = json.loads(open("AnonXMusic/assets/quran.json").read())["s"] # ملف القران الي بستخدمه بتاع @devzaid 
...
async def show_quran(c,cq):
    ...
    i = int(data[2])
    try:
        await Anony.join_call(cq.message.chat.id, cq.message.chat.id, quran[number]["sounds"][i]["url"])
